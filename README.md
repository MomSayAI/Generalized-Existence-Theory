...
This paper presents **Generalized Existence Theory (GET)**, which seeks to address the most fundamental questions in philosophy and physics: **How does existence come to be, and how does it evolve?**
...
| Solidified energy | \(E_{\text{solid}}\) | Locked structural assets | Experience, knowledge, skills — hard to access when drained |
| Active energy | \(E_{\text{active}}\) | Usable disposable reserves | Current attention, ATP, vitality, cash flow |
| Solidified time constant | \(\tau_{\text{solid}}(L)\) | Depreciation rate of structural assets | How long before you forget without practice |
| Active time constant | \(\tau_{\text{active}}(L)\) | Consumption rate of active reserves | How long your current energy lasts |
| Energy capture efficiency | \(\eta(L)\) | Conversion efficiency from input to usable energy | Learning efficiency, metabolic efficiency |
| Survival criterion | \(\frac{E_{\text{active}}}{\tau_{\text{active}}(L)} + \eta(L) \cdot e(t) \geq 0\) | Can you continue? | "Can I keep going?" |
...
- \(\Omega^{(M)}\): The complete state of any existent form \(M\) (what we observe as "reality" at any moment — particles, stars, life, consciousness, civilizations)
- \(\Phi\): The generative function — given a set of protocols \(L\), it outputs the full evolutionary trajectory of \(M\)
...
\[
\Omega_{\mathcal{S}}^{(M)}(t) = \Phi_{\mathcal{S}}\big(L,\; E_{\text{solid}}(t),\; E_{\text{active}}(t),\; e(t),\; t\big)
\]

Where:
- \(E_{\text{solid}}(t)\) — solidified structural assets (locked energy, long-term "body")
- \(E_{\text{active}}(t)\) — active disposable reserves (cash flow, ATP, attention)
...
- \(\mathcal{S}\) — species-specific signature (core protocol fingerprint \(C_{\mathcal{S}}\))
...
1. **Trial** — Probe a new protocol from candidate set \(\mathscr{P}_{d+1}\) within a qualified window
2. **Capture** — Bind it temporarily using high existence capital
3. **Solidify** — Lock it permanently if energy and time thresholds are met
...
| Freeze | Channel closes at \(T_0 + \tau_{\text{inflation}}\) | All ongoing processes permanently freeze |
| Three categories | Born in that instant | **A**: Ordinary matter (completed) <br>**B**: Dark matter (interrupted, frozen) <br>**C**: Dark energy (never reached threshold, uniform) |
...
2. A stable existent cannot be fully described by \(\Omega^{(M)} = \Phi(L)\) or violates the survival criterion  
3. Dark matter exhibits any non-gravitational interaction (cross-section \(> 0\))  
4. Dark energy's equation of state deviates from \(w \equiv -1\) (e.g., \(5\sigma\) evidence)  
5. Superluminal causal energy/information transfer is confirmed  

**No retreat. No rescue. Just judgment.**

GET invites anyone possessing basic logical ability and modern scientific common sense to attempt violent falsification — using AGI or other means, if deemed necessary. Corollary 22 provides the explicit ultimate conditions.
...
**Logic?** Follow the chain: Bedrocks → Definitions → Axioms → Corollaries.

**Apply?** Pick a domain and map your system using the GET framework.

**Falsify?** Hunt counterexamples against the five falsification conditions.

**Origin?** The *Voyaging* universe (fiction) is where this grammar was first seen.
